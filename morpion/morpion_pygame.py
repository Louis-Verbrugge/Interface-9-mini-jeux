...
def affiche_fin_de_partie(plateau, symboles, nb_victoire):
    ...
    while run_affiche:
        ...
        score_ordinateur = pygame.font.SysFont("bahnschrift", 50).render(f"{qui_gagne_} {str(max(nb_victoire[0], nb_victoire[1]))} - {min((nb_victoire[0], nb_victoire[1]))}", True, "white")
        ...
        #screen.blit(score_utilisateur, score_u)
...
WIDTH, HEIGHT = 1600, 1000   ###WIDTH, HEIGHT = 800, 600
size = width, height = 1600, 1000 ### size = width, height = 800, 600
screen = display.set_mode(size)




hauteur_height = screen.get_height()
largeur_width = screen.get_width()
...
draw.rect(screen, RED, (HEIGHT-120, WIDTH-220, HEIGHT-80, WIDTH-180), 1)  # A SUPPRIMER APRES TEST
...
def main(nb_victoire):
    ...
    if verifier_victoire(plateau, symboles):
        ...
        affiche_fin_de_partie(plateau, symboles, nb_victoire)
