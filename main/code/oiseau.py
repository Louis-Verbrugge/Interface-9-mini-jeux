...
def rebond_mur(SCREEN, birds, possition, score):
    ...
    if 0 >= birds.get_x() - birds.get_rayon():   # JE vrif si la birds touche le mur a gauche
        ...
        score+=1
        SCREEN.fill((0, 0, 0))
        spick_plafond_and_sol_score(SCREEN, possition, score)
...
def spick_plafond_and_sol_score(SCREEN, possition, score):
    
    # affiche spick en haut et en bas:
    for i in range (14):
        # plafond:
    ...
    # affiche spick sur les murs:
    spick_mur(SCREEN, possition)


    #affiche_resultat de l'utilisateur:
...
def spick_mur(SCREEN, possition_spick):
...
def des_spick_mur(SCREEN, possition_spick):
    for i in range (len(possition_spick)):
...
def saut(SCREEN, birds, possition, score):
    ...
    while pos_y_a_atteindre <= birds.get_y():
        ...
        possition, score = rebond_mur(SCREEN, birds, possition, score)
...
def game_oiseau(SCREEN):
    

    
    Vx = 0.2  # VITESSE X  # cette valeur doit etre constant ou augmente pour la difficulter
    Vy = 0.22  # VITESSE y
    ...
    while run_debut:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                run_game = True
                defaire_or_continue, score  = saut(SCREEN, birds, possition, 0) # 0 car pour le moment il y a un score de 0
        ...
        while run_game:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    continuee, score = saut(SCREEN, birds, possition, score)
...
def main_oiseau(): 
    ...
    while fin != "quitte":
        fin, score = game_oiseau(SCREEN)
        if fin != "quitte":
            fin = relance_game(SCREEN, score)
        pygame.display.update()
